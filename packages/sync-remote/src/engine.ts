...
export class SyncEngine {
  ...
  constructor(private readonly options: SyncEngineOptions) {
    this.repository = new SyncRepository(options.remote, options.crypto);
  ...
  async connect(): Promise<ProtocolMetadata> {
    const { remote, crypto, store, masterKey } = this.options;
  ...
  private async uploadPendingAttachments(
    ...
    for (const hash of await this.options.queue.pendingAttachments()) {
      ...
      if (await this.options.remote.exists(path)) {
      ...
      await this.options.remote.put(path, encrypted);
      await this.options.remote.verifyUpload(path, encrypted.length);
      await this.options.queue.acknowledgeAttachment(hash);
      uploaded++;
      void syncKey;
    }
  ...
  private async downloadPendingAttachments(): Promise<number> {
    ...
    for (const hash of await this.options.queue.pendingDownloads()) {
      ...
      const raw = await this.options.remote.getIfExists(attachmentPath(hash));
      if (!raw) {
  ...
  async rebuildRemote(
    ...
    const { store, masterKey, remote } = this.options;
    const deviceId = await store.getDeviceId();
    const generation = newGeneration(deviceId);

    const staging = `.staging-${generation}`;
    await remote.makeDirectory(staging + "/");
    ...
    // devices/ and objects/ are directories and protocol.json is a file, and
    // a store whose backend has no directory move walks the tree instead —
    // which is why this goes through moveRecursive rather than move. Moving a
    // directory with the file-only path used to write its listing into a file.
    ...
    for (const path of [PATHS.devices, PATHS.objects, PATHS.protocol]) {
      if (await remote.exists(path)) {
        await remote.makeDirectory(retired + "/");
        await remote.moveRecursive(path, `${retired}/${path}`);
      }
    }
    for (const path of [PATHS.devices, PATHS.objects, PATHS.protocol]) {
      if (await remote.exists(`${staging}/${path}`)) {
        await remote.moveRecursive(`${staging}/${path}`, path);
      }
    }
  ...
  async pruneAttachments(
    ...
    const entries = await this.options.remote.list(PATHS.attachments + "/");
    ...
    for (const entry of entries) {
      ...
      const modified = entry.modifiedAt ?? Number.NaN;
      ...
      await this.options.remote.delete(attachmentPath(hash));
      removed++;
    }
    return removed;
  }

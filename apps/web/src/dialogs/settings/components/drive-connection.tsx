...
/**
 * Connecting Google Drive, OneDrive or Dropbox directly.
 *
 * This is the second-choice route and the form says so. If the provider's own
 * desktop client is installed, the folder provider reaches the same account
 * with no sign-in, no registration and no third-party network access from
 * Openotes at all. This exists for machines that have no such client.
 *
 * The registration is the user's own: Openotes has no OAuth application, so
 * there is no client id to ship and nothing for a provider to revoke out from
...
export function DriveConnectionPanel() {
  ...
  useEffect(() => {
    if (!provider || provider === "webdav" || provider === "folder") return;
  ...
  return (
    <Flex sx={{ flexDirection: "column", gap: 1, flex: 1 }}>
      <Text variant="subBody" sx={{ color: "var(--paragraph-secondary)" }}>
        If you already run {setup.label}&apos;s desktop client, use{" "}
        <b>a folder on this machine</b> instead — it reaches the same account
        without a sign-in, and Openotes never talks to {setup.label} at all.
      </Text>

      <Text variant="subBody" sx={{ fontWeight: "bold", mt: 1 }}>
        Register your own app in {setup.label}
      </Text>
      <Flex as="ol" sx={{ flexDirection: "column", gap: 1, pl: 3, m: 0 }}>
        ...
        <Text as="li" variant="subBody">
          Redirect URI: <code>http://{setup.loopbackHost}/oauth/callback</code>{" "}
          — the port changes each time, which every provider allows for
          loopback.
        </Text>
        <Text as="li" variant="subBody">
      ...
      <Field
        ...
        helpText={
          config?.connected
            ...
            : "What encrypts your notes. It never leaves this machine, so it cannot be recovered from the provider."
        }
        value={passphrase}
